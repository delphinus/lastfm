...
LFM.prototype = {
    ...
    ,init: function( options, item ) {
        ...
        // 今までに作成したLFMオブジェクトの一覧を走査して
        // 対象の要素にすでにLFMオブジェクトがすでに存在するならばそれを使う 
        ...
        this.imgSize = this.settings.artSize == 'small' ? 0 :
            this.settings.artSize == 'medium' ? 1 :
            this.settings.artSize == 'large' ? 2 :
            0
            ;
        this.settings.updateInterval = parseNum(
            this.settings.updateInterval, this.defaults.updateInterval );
    }
    ...
    ,updatetracks: function() {
        ...
        this.t = setepoch( this.settings.updateInterval );

        // 次の更新を予約
        if ( this.settings.autoUpdate ) {
            // 時刻表示ラベルの追加
            if ( ! this.timerLabel ) {
                this.timerLabel = $( '<div/>', { className: 'lfm_update' } )
                ...
                this.timer.sub = setInterval(
                    scope( function() {
                        ...
                        if ( sec <= 0 ) {
                            ...
                            text = new Date( this.t.next * 1000 )
        ...
        $.getJSON( 'http://ws.audioscrobbler.com/2.0/?callback=?'
            ,{
                ...
                ,limit: this.settings.number
                ,nowplaying: true
                // すでに読んだことがあるならば、
                // 一番新しいもの以降を読む
                ,from: this.tracks.length > 0 ?
                            this.tracks[ this.tracks[0].uts == 0 ? 1 : 0 ].uts
                    : 0
            }
            ,scope( function( data ) {
    ...
    ,displaytrack: function( i, item ) {
        var interval = this.settings.drawDelay
                ? this.settings.drawInterval * i : 0
        ...
        // 曲名・アーティスト名・アルバム名を表示
        $.each( 'song artist album'.split( /\s/ ), function() {
            $( '.lfm_' + this, track.item ).text( track[ this ] );
        } );
        ...
        // 溢れた分を削除
        if ( this.tracks.length + 1 > this.settings.number ) {
        ...
        if ( showArtistImage && this.settings.showArtistArt ) {
            $.getJSON( 'http://ws.audioscrobbler.com/2.0/?callback=?'
                ,{
...
function parseNum( i, def ) {
    i = i + '';
    return i.match( /(\d+)h$/i ) ? RegExp.$1 * 3600 :
...
function setepoch( int ) {
    ...
    t = {
        ...
        ,next: new Date( now.getTime() + int * 1000 )
    };
